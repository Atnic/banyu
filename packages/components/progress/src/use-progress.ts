import type {ProgressVariantProps, SlotsToClasses, ProgressSlots} from "@jala-banyu/theme";
import type {PropGetter} from "@jala-banyu/system";
...
import {clampPercentage, clsx, dataAttr} from "@jala-banyu/shared-utils";
import {ReactRef} from "@jala-banyu/react-utils";
import {mergeProps} from "@react-aria/utils";
import {useMemo, useCallback, useRef} from "react";
import {useIsMounted} from "@jala-banyu/use-is-mounted";
...
interface Props extends HTMLBanyuProps<"div"> {
  ...
  floatingLabelPlacement?: "top" | "bottom";
...
export function useProgress(originalProps: UseProgressProps) {
  ...
  const chipRef = useRef<HTMLBanyuProps>(null);
  ...
  const color = originalProps.color === "default" ? "primary" : originalProps.color ?? "primary";
  const getChipProps: PropGetter = () => ({
    ...
    "aria-disabled": originalProps.isDisabled ?? false,
    classNames: {
      content: "p-0",
    },
  });
