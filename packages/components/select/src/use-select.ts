import type {SelectSlots, SelectVariantProps, SlotsToClasses} from "@jala-banyu/theme";
import type {HiddenSelectProps} from "./hidden-select";

import {DOMAttributes, HTMLBanyuProps, mapPropsVariants, PropGetter} from "@jala-banyu/system";
import {select} from "@jala-banyu/theme";
import {ReactRef, useDOMRef, filterDOMProps} from "@jala-banyu/react-utils";
import {useMemo, useCallback, useRef, Key, ReactNode, useEffect} from "react";
import {ListboxProps} from "@jala-banyu/listbox";
...
import {PopoverProps} from "@jala-banyu/popover";
import {ScrollShadowProps} from "@jala-banyu/scroll-shadow";
import {
  MultiSelectProps,
  useMultiSelect,
  useMultiSelectState,
} from "@jala-banyu/use-aria-multiselect";
...
interface Props<T> extends Omit<HTMLBanyuProps<"select">, keyof SelectVariantProps> {
  /**
   * Ref to the DOM node.
   */
...
export function useSelect<T extends object>(originalProps: UseSelectProps<T>) {
  ...
  const labelPlacement = useMemo<SelectVariantProps["labelPlacement"]>(() => {
    return originalProps.labelPlacement;
