...
const defaultProps = {
  ...
  className:
    "w-full max-w-[260px] border-lg px-1 py-2 rounded-lg border-neutral-200 dark:border-neutral-100",
};
...
const Template = ({...args}: ListboxProps) => (
  <Listbox aria-label="Actions" onAction={(key: Key) => alert(key)} {...args}>
...
const DynamicTemplate = ({...args}: ListboxProps<Item>) => (
  <Listbox aria-label="Dynamic Actions" items={items} onAction={(key) => alert(key)} {...args}>
    {(item) => <ListboxItem key={item.key}>{item.label}</ListboxItem>}
  </Listbox>
);

const DisabledKeysTemplate = ({...args}: ListboxProps) => (
  <Listbox
    aria-label="Actions"
...
const SingleSelectionTemplate = ({...args}: ListboxProps) => {
...
const MultipleSelectionTemplate = ({...args}: ListboxProps) => {
  const [selected, setSelected] = React.useState<Selection>(new Set(["text"]));
...
const WithEndContentTemplate = ({disableAnimation, ...args}: ListboxProps) => {
...
const WithTopContentTemplate = ({...args}) => {
...
const WithBottomContentTemplate = ({...args}) => {
  const [values, setValues] = React.useState<Selection>(new Set(["1", "3", "5"]));
...
const WithDescriptionTemplate = ({disableAnimation, ...args}: ListboxProps) => {
  const iconClasses = "text-2xl text-secondary pointer-events-none flex-shrink-0";
...
const WithSectionsTemplate = ({disableAnimation, ...args}: ListboxProps) => {
  const iconClasses = "text-2xl text-secondary pointer-events-none flex-shrink-0";
...
const CustomWithClassNamesTemplate = ({disableAnimation, ...args}: ListboxProps) => {
      ...
      >
        <div className="flex flex-col gap-1">
          ...
          <div className="px-2 py-1 rounded-small bg-default-100 group-data-[hover=true]:bg-default-200">
            <span className="text-tiny text-default-600">@jala-banyu/react@2.0.10</span>
