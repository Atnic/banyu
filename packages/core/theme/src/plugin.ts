...
import {ConfigTheme, ConfigThemes, DefaultThemeType, BanyuThemePluginConfig} from "./types";
import {lightLayout, darkLayout, defaultLayout} from "./default-layout";
import {baseStyles} from "./utils/classes";

const DEFAULT_PREFIX = "jds";
  ...
  for (const [themeName, {extend, layout, colors}] of Object.entries(themes)) {
    ...
    for (const [colorName, colorValue] of Object.entries(flatColors)) {
      ...
      try {
        ...
        const banyuThemeColorVariable = `--${prefix}-${colorName}`;
        const banyuThemeOpacityVariable = `--${prefix}-${colorName}-opacity`;
        ...
        resolved.colors[colorName] = ({opacityVariable, opacityValue}) => {
          ...
          if (!isNaN(+opacityValue)) {
            return `hsl(var(${banyuThemeColorVariable}) / ${opacityValue})`;
          }
          // if no opacityValue was provided (=it is not parsable to a number)
          // the banyuThemeOpacityVariable (opacity defined in the color definition rgb(0, 0, 0, 0.5)) should have the priority
          // over the tw class based opacity(e.g. "bg-opacity-90")
          // This is how tailwind behaves as for v3.2.4
          if (opacityVariable) {
            return `hsl(var(${banyuThemeColorVariable}) / var(${banyuThemeOpacityVariable}, var(${opacityVariable})))`;
          }

          return `hsl(var(${banyuThemeColorVariable}) / var(${banyuThemeOpacityVariable}, 1))`;
        };
      } catch (error: any) {
...
export const banyuTheme = (config: BanyuThemePluginConfig = {}): ReturnType<typeof plugin> => {
  const {
    themes: themeObject = {},
    defaultTheme = "light",
    layout: userLayout,
